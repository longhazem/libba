...
local function GetPlaceIcon(placeId)
    ...
    local icon = (ok and url) or "rbxassetid://5107154082"
...
-- ─── Lucide-style Icon Paths (simplified SVG → ImageLabel approach) ──────────

local ICON_SETTINGS = "rbxassetid://7733960981"   -- gear/settings icon
local ICON_BACK     = "rbxassetid://7733658504"    -- arrow-left icon
local ICON_CHECK    = "rbxassetid://7733715400"    -- check icon

-- Fallback: if the library has icon loading
...
local function Create3DBubble(player, message, timestamp)
    ...
    if not GlobalChat.Settings.ShowUsername then
        -- Message only, no name in bubble
    end
...
function GlobalChat:AddMessage(data)
    ...
    if showUsername then
        ...
        displayName = MaskUsername(data.displayName)
    end
...
function GlobalChat:SendMessage(message)
    ...
    local data = {
        userId      = LP.UserId,
        username    = self.Settings.ShowUsername and LP.Name or MaskUsername(LP.Name),
        displayName = self.Settings.ShowUsername and LP.DisplayName or MaskUsername(LP.DisplayName),
        message     = message,
        timestamp   = os.time(),
        gameId      = game.PlaceId,
...
function GlobalChat:ToggleSettings()
    ...
    if self.SettingsOpen then
        ...
        -- Now reveal items one by one with stagger
        for i, item in ipairs(self.SettingsItems) do
...
function GlobalChat:BuildSettingsPanel()
    ...
    -- Overlay frame that covers the chat area
    local overlay = New("Frame", {
        BackgroundColor3 = L.Scheme.BackgroundColor,
        BorderSizePixel  = 0,
        Position         = UDim2.new(0, 0, 0, 0),
        Size             = UDim2.new(1, 0, 1, -44),  -- leave input area visible? No, cover all
        ZIndex           = 10,
        ClipsDescendants = true,
        Visible          = false,
    ...
    -- Full size to cover messages + input
    ...
    self:CreateSettingsToggle(content, yOffset, "Show Username & Avatar", "ShowUsername", function(val)
        -- Sync avatar with username
        self.Settings.ShowAvatar = val
        -- Rebuild messages
...
function GlobalChat:CreateWindow()
    ...
    -- Hover animation for settings icon
